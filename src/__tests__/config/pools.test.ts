...
describe('Config pools', () => {
  ...
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'BNB'))(
    ...
    async (pool) => {
      ...
      expect(rewardTokenAddress.toLowerCase()).toBe(pool.earningToken.address[25].toLowerCase())
    },
  )
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'BNB'))(
    ...
    async (pool) => {
      ...
      expect(stakingTokenAddress.toLowerCase()).toBe(pool.stakingToken.address[25].toLowerCase())
    },
  )
})
