...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, nnPerBlock, startBlockHarvest] =
    pid || pid === 0
  ...
  const NNPerBlock = nnPerBlock? new BigNumber(nnPerBlock).div(BIG_TEN.pow(9)) : BIG_ZERO
